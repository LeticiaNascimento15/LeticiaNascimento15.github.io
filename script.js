...
const urlFactory = recurso => `http://localhost:80/${recurso}/index.php` 
...
function cadastrarExemplar() {
  ...
  const l = new Livro(issn, titulo, autor, editora, ano, genero, local);
  inserirExemplar();


}

async function inserirExemplar (livro){
  const resp = await fetch(urlFactory("livro"), {
